-- ==============================================================================
-- NEXUS WMS — SETUP โปรเจกต์ Supabase ใหม่ (รันทีเดียวจบใน SQL Editor)
-- รวม: schema ทั้งหมด + TMS/branch/FEFO/transfers + fleet + multi-drop + seed
-- ปลอดภัย: idempotent (IF NOT EXISTS / ON CONFLICT) รันซ้ำได้
-- ==============================================================================
...
-- ============ [2/5] MIGRATION: TMS / BRANCH / FEFO / TRANSFERS ============
-- ==============================================================================
...
-- ============ [3/5] MIGRATION: FLEET + VEHICLE/DRIVER ON ORDERS ============
-- ==============================================================================
...
-- ============ [4/5] SEED: ผู้ใช้ Super Admin + ขนส่ง ============

-- Super Admin: user "arm" / รหัส "arm080831" (bcrypt)
INSERT INTO app_users (id, org_id, username, email, role, status, password_hash, allowed_branches, allowed_owners, last_login)
VALUES (
  'U-arm-superadmin',
  '00000000-0000-0000-0000-000000000001',
  'arm', 'arm@wms360.pro', 'Super Admin', 'Active',
  '$2b$10$hOeeIBadivRJmzUuRZaPIOlqnxveMB/965U7DWBwTG58.c3H3pkay',
  ARRAY['*'], ARRAY['*'], ''
) ON CONFLICT (id) DO NOTHING;

-- ขนส่ง (รถบริษัท = default + trigger เข้า TMS; ที่เหลือ = 3PL)
INSERT INTO carriers (org_id, code, name, tracking_url_template, is_default, status) VALUES
('00000000-0000-0000-0000-000000000001', 'OWN_FLEET', 'รถขนส่งบริษัท (จัดส่งเอง)', '', true,  'ACTIVE'),
('00000000-0000-0000-0000-000000000001', 'FLASH', 'Flash Express',        'https://www.flashexpress.com/fle/tracking?se={trackingNo}',             false, 'ACTIVE'),
('00000000-0000-0000-0000-000000000001', 'KEX',   'Kerry Express',        'https://th.kerryexpress.com/th/track/?track={trackingNo}',              false, 'ACTIVE'),
('00000000-0000-0000-0000-000000000001', 'JNT',   'J&T Express',          'https://www.jtexpress.co.th/index/query/gzquery.html?bills={trackingNo}', false, 'ACTIVE'),
('00000000-0000-0000-0000-000000000001', 'EMS',   'ไปรษณีย์ไทย (EMS)',    'https://track.thailandpost.co.th/?trackNumber={trackingNo}',            false, 'ACTIVE'),
('00000000-0000-0000-0000-000000000001', 'SPX',   'Shopee Express (SPX)', 'https://spx.co.th/track?{trackingNo}',                                  false, 'ACTIVE'),
('00000000-0000-0000-0000-000000000001', 'NINJA', 'Ninja Van',            'https://www.ninjavan.co/th-th/tracking?id={trackingNo}',                false, 'ACTIVE'),
('00000000-0000-0000-0000-000000000001', 'BEST',  'BEST Express',         'https://www.best-inc.co.th/track?bills={trackingNo}',                   false, 'ACTIVE')
ON CONFLICT DO NOTHING;

-- ============ [5/5] RELOAD SCHEMA CACHE (กัน PGRST205) ============
NOTIFY pgrst, 'reload schema';

SELECT '✅ NEXUS WMS setup completed — login: arm / arm080831' AS status;
